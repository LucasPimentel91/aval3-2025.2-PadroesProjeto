...
public class GerenciadorDocumentoModel {
    ...
    public void macroAlterarEAssinar(Documento doc, String conteudo) throws FWDocumentException {
        ...
        try {
            ComandoMacro macro = new ComandoMacro("Macro: Alterar e Assinar");

            macro.addCommand(new ComandoEditarConteudo(this, doc, conteudo));
            macro.addCommand(new ComandoAssinar(this, doc));
    ...
    public void macroPriorizar(Documento doc) throws FWDocumentException {
        ...
        try {
            ComandoMacro macro = new ComandoMacro("Macro: Priorizar (Urgente + Assinar)");

            macro.addCommand(new ComandoMarcarUrgente(this, doc));
            macro.addCommand(new ComandoAssinar(this, doc));
    ...
    public void restore(DocumentoMemento memento) throws FWDocumentException {
        ...
        Documento doc = memento.documentoRef();

        if (doc == null) return;

        int idx = memento.index();

        if (idx >= 0 && idx < repositorio.size()) {
            repositorio.set(idx, doc);
        }

        if (memento.conteudo() != null) {
            doc.setConteudo(memento.conteudo());
        }

        if (memento.urgente() != null) {
            restoreUrgente(doc, memento.urgente());
        }
